...
ReadCameraInfoFile(void)
{
  ...
  if (!fp) {
    fprintf(stderr, "Unable to open camera info file: %s\n", filename);
    return 0;
  }
  ...
  while (fgets(buffer, buffer_size, fp)) {
    ...
    if (!strcmp(keyword, "camera_number")) {
      ...
      else if (!strcmp(keyword, "fov_max")) camera->SetMaxFov(value);
    }
  }
...
ReadImageSegmentFile(RNArray<GSVPanorama *>& panoramas)
{
  ...
  FILE *fp = fopen(filename, "r");
  if (!fp) {
    fprintf(stderr, "Unable to open segment info file: %s\n", filename);
    return 0;
  }
...
ReadImagePoseFile(const RNArray<GSVPanorama *>& panoramas)
{
  ...
  FILE *fp = fopen(filename, "r");
  if (!fp) {
    fprintf(stderr, "Unable to open image pose file: %s\n", filename);
    return 0;
  }
...
ReadLaserObjFile(int laser_index, RNArray<GSVScanline *>& scanlines, RNArray<GSVScan *>& scanline_scans, RNBoolean read_points)
{
  ...
  R3Point scanline_points[max_scanline_points];
  while (fgets(buffer, buffer_size, fp)) {
    line_count++;
    if (buffer[0] == '\0') continue;
    if (buffer[0] == '#') continue;
    if (buffer[0] == 'v') {
      ...
      if (scanline) {
        if (num_scanline_points < max_scanline_points) {
          scanline_points[num_scanline_points].Reset(x, y, z);
          num_scanline_points++;
        }
      }
    }
        ...
        if (read_points) {
          ...
          if (num_scanline_points > 25) {
            for (int i = num_scanline_points - 25; i < num_scanline_points; i++) {
              scanline_points[i][2] = scanline_points[num_scanline_points-25][2];
            }
          }
          scanline->SetPointPositions(scanline_points, num_scanline_points);
        }
      ...
      // Get/create scan
      GSVScan *scan = NULL;
      while (segment->NScans() <= laser_index) 
        segment->InsertScan(new GSVScan());
      scan = segment->Scan(laser_index);

      // Insert scan into laser
      if (scan->Laser() == NULL) laser->InsertScan(scan);
      assert(scan->Laser() == laser);
