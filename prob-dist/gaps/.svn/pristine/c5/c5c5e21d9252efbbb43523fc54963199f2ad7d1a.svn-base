...
static char *output_feature_grid_directory = NULL;
static char *output_feature_grid_name = NULL;
...
static int create_mesh_correspondences = 0;
...
static int show_triangles[2] = { 0, 0 };
...
ReadOptions(GSVPoseOptimization *optimization, const char *filename)
{
  ...
  while (fgets(buffer, 4096, fp)) {
    ...
    else if (!strcmp(keyword, "create_mesh_correspondences"))
      create_correspondences |= create_mesh_correspondences = atoi(value);
...
WriteOptions(GSVPoseOptimization *optimization, const char *filename)
{
  ...
  fprintf(fp, "create_mesh_correspondences = %d\n", create_mesh_correspondences);
...
static int
WriteFeatureGrids(GSVPoseOptimization *optimization, 
  const char *output_directory = NULL, const char *output_name = NULL, 
  const char *parameterization = "DA", RNLength world_sigma = 0.5)
{
  // Start statistics
  RNTime start_time;
  start_time.Read();
  ...
  // Get scene
  GSVScene *scene = optimization->scene;
  if (!scene) return 0;

  // Get laser image directory name
  char laser_image_directory[4096];
  sprintf(laser_image_directory, "%s/laser_images", scene->CacheDataDirectoryName());

  // Resolve names
  if (!output_directory) output_directory = laser_image_directory;
  if (!output_name) output_name = "ScanFeatures";

  // Create output directory
  char cmd[4096];
  sprintf(cmd, "mkdir -p %s", output_directory);
  system(cmd);

  // Compute convenient variables
  RNLength world_sigma_squared = world_sigma * world_sigma;
  RNLength max_distance_squared =3.0 * world_sigma_squared;
  RNLength denom = -2.0 * world_sigma_squared;

  // Write feature grids
  for (int ir = 0; ir < scene->NRuns(); ir++) {
    GSVRun *run = scene->Run(ir);

    // Write feature grids for run
    for (int is = 0; is < run->NSegments(); is++) {
      GSVSegment *segment = run->Segment(is);
      for (int ia = 0; ia < segment->NScans(); ia++) {
        GSVScan *scan = segment->Scan(ia);
        ...
        R2Grid height_grid;
        sprintf(image_name, "%s/%s/%02d_%02d_%s_Height.grd", laser_image_directory, run->Name(), is, ia, parameterization);
        if (!height_grid.Read(image_name)) { scan->ReleasePoints(); return 0; }

        // Initialize feature grid
        R2Grid feature_grid(height_grid);
        feature_grid.Clear(0.0);

        // Write features to grid
        int count = 0;
        for (int k = 0; k < optimization->NFeatures(); k++) {
          GSVFeature *feature = optimization->Feature(k);
          ...
          if (scanline->Scan() != scan) continue;
          int scanline_index = scanline->ScanIndex();
          R3Point position = feature->scan_position;
          // R3Point position = scanline->PointPosition(point_index);

          // Rasterize values for neighborhood around feature point
          for (int i = -20; i <= 20; i++) {
            int s_index = scanline_index + i;
            if ((s_index < 0) || (s_index >= scan->NScanlines())) continue;
            GSVScanline *s = scan->Scanline(s_index);
            for (int j = -20; j <= 20; j++) {
              int p_index = point_index + j;
              if ((p_index < 0) || (p_index >= scanline->NPoints())) continue;
              R3Point p = s->PointPosition(p_index);

              // Compute value 
              RNLength distance_squared = R3SquaredDistance(p, position);
              if (distance_squared > max_distance_squared) continue;
              RNScalar value = exp(distance_squared/denom);

              // Get grid position in appropriate parameterization
              RNScalar x = s_index;
              RNScalar y = p_index;
              if (parameterization[0] == 'D') {
                x = s->TravelDistance();
              }
              if (parameterization[1] == 'A') {
                const GSVPose& pose = s->Pose();
                const R3Point& viewpoint = pose.Viewpoint();
                const R3Vector& towards = pose.Towards();
                const R3Vector& up = pose.Up();
                R3Vector v = p - viewpoint;
                v.Normalize();
              ...
              else if (parameterization[1] == 'H') {
                RNLength height = p.Z() - s->EstimatedGroundZ();
                y = height;
              }

              // Get position in grid coordinates
              R2Point g = feature_grid.GridPosition(x,y);
              int ix = (int) (g.X() + 0.5);
              int iy = (int) (g.Y() + 0.5);
              if ((ix < 0) || (ix >= feature_grid.XResolution())) continue;
              if ((iy < 0) || (iy >= feature_grid.YResolution())) continue;

              // Check if best value so far at position
              RNScalar old_value = feature_grid.GridValue(ix, iy);
              if ((old_value != R2_GRID_UNKNOWN_VALUE) && (value > old_value)) {
                feature_grid.SetGridValue(ix, iy, value);
                count++;
              }
            }
          }
        ...
        if (count > 0) {
          // Create output directory
          char cmd[4096];
          sprintf(cmd, "mkdir -p %s/%s", output_directory, run->Name());
          system(cmd);

          // Write grid
          sprintf(image_name, "%s/%s/%02d_%02d_%s_%s.grd", output_directory, run->Name(), is, ia, parameterization, output_name);
          if (!feature_grid.Write(image_name)) return 0;
        }
  ...
  // Print statistics
  if (print_verbose) {
    printf("Wrote feature grids to %s ...\n", output_directory);
    printf("  Time = %.2f seconds\n", start_time.Elapsed());
    printf("  # Features = %d\n", optimization->NFeatures());
    fflush(stdout);
  }
...
CreateCorrespondences(GSVPoseOptimization *optimization)
{
  ...
  else if (create_mesh_correspondences) {
    if (!optimization->CreateMeshCorrespondences()) return 0;
  }
...
ReadScanData(GSVScan *scan, RNScalar timestamp) 
{
  ...
  // Read segmentation grid
  if (1 || load_segments || load_textures) {
    sprintf(image_name, "%s/%s/%02d_%02d_SA_Scanline.grd", image_directory, run->Name(), segment_index, scan_index);
    scan_data->scanline_grid.Read(image_name);
    sprintf(image_name, "%s/%s/%02d_%02d_SA_PositionX.grd", image_directory, run->Name(), segment_index, scan_index);
    scan_data->position_x_grid.Read(image_name);
    sprintf(image_name, "%s/%s/%02d_%02d_SA_PositionY.grd", image_directory, run->Name(), segment_index, scan_index);
    scan_data->position_y_grid.Read(image_name);
    sprintf(image_name, "%s/%s/%02d_%02d_SA_PositionZ.grd", image_directory, run->Name(), segment_index, scan_index);
    scan_data->position_z_grid.Read(image_name);
    sprintf(image_name, "%s/%s/%02d_%02d_SA_Timestamp.grd", image_directory, run->Name(), segment_index, scan_index);
    scan_data->timestamp_grid.Read(image_name);
  }

  // Read segmentation grid
  if (load_segments) {
    sprintf(image_name, "%s/%s/%02d_%02d_SA_%s.grd", image_directory, run->Name(), segment_index, scan_index, segment_grid_name);
    scan_data->segment_id_grid.Read(image_name);
  } 

  // Read texture
  if (load_textures) {
    // sprintf(image_name, "%s/%s/%02d_%02d_DA_Color.bmp", image_directory, run->Name(), segment_index, scan_index);
    // scan_data->texture = new R2Texture(image_name);
    R2Grid red_grid, green_grid, blue_grid;
    sprintf(image_name, "%s/%s/%02d_%02d_SA_ColorRed.grd", image_directory, run->Name(), segment_index, scan_index);
    red_grid.Read(image_name);
    sprintf(image_name, "%s/%s/%02d_%02d_SA_ColorGreen.grd", image_directory, run->Name(), segment_index, scan_index);
    green_grid.Read(image_name);
    sprintf(image_name, "%s/%s/%02d_%02d_SA_ColorBlue.grd", image_directory, run->Name(), segment_index, scan_index);
    blue_grid.Read(image_name);
    R2Image *image = new R2Image(red_grid.XResolution(), red_grid.YResolution(), 3);
    ...
    scan_data->texture = new R2Texture(image);
    scan_data->texture->Load();
  }

  // Read planar grids
  if (load_planar_grids) {
    for (int j = 0; j < 100; j++) {
      sprintf(image_name, "gsv_data/planar_images/%s/%02d_%02d_%06d_ST_Displacement.grd", run->Name(), segment_index, scan_index, j);
...
ReleaseScanData(GSVScan *scan)
{
  ...
  // Delete texture
  if (scan_data->texture) {
    scan_data->texture->Unload();
    delete scan_data->texture;
  }
  ...
  for (int ir = 0; ir < scene->NRuns(); ir++) {
    ...
    for (int is = 0; is < run->NSegments(); is++) {
      ...
      for (int ia = 0; ia < segment->NScans(); ia++) {
        ...
        if (!scan_data) continue;
        if (scan->NScanlines() == 0) continue;
        GSVMesh *mesh = scan_data->mesh;
        if (!mesh) continue;

        // Set texture and material
        if (scan_data->texture && scan_data->texture->Image()) {
          // Set texture
          glColor4d(1.0, 1.0, 1.0, 1.0);
          scan_data->texture->Draw();
          RNScalar max_travel_distance = scan->Scanline(scan->NScanlines()-1)->TravelDistance();
          if (max_travel_distance == 0) continue;

          // Set texture transformation
          glMatrixMode(GL_TEXTURE);
          glScaled(1.0 / max_travel_distance, 1.0 / RN_PI, 1.0);
          glMatrixMode(GL_MODELVIEW);
        }
        else {
          // Set material
          glEnable(GL_LIGHTING);
          GLfloat material[4] = { (GLfloat) color[0], (GLfloat) color[1], (GLfloat) color[2], (GLfloat) 1.0 };
        ...
        for (int i = 0; i < mesh->NFaces(); i++) {
          ...
          GSVMeshVertex *v0 = mesh->VertexOnFace(face, 0);
          GSVMeshVertex *v1 = mesh->VertexOnFace(face, 1);
          GSVMeshVertex *v2 = mesh->VertexOnFace(face, 2);
          GSVScanline *scanline0 = mesh->VertexScanline(v0);
          GSVScanline *scanline1 = mesh->VertexScanline(v1);
          GSVScanline *scanline2 = mesh->VertexScanline(v2);
          RNScalar s0 = scanline0->TravelDistance();
          RNScalar s1 = scanline1->TravelDistance();
          RNScalar s2 = scanline2->TravelDistance();
          RNScalar t0 = mesh->VertexLaserAngle(v0);
          RNScalar t1 = mesh->VertexLaserAngle(v1);
          RNScalar t2 = mesh->VertexLaserAngle(v2);
...
#if 0
static void 
DrawRayIntersections(int side, RNBoolean transform, int color_scheme)
{
  ...
  // Set OpenGL modes
  glEnable(GL_LIGHTING);
  static GLfloat material[] = { 1.0, 1.0, 1.0, 1.0 };
  glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, material); 
  ...
  for (int ia = 0; ia < segment->NScans(); ia++) {
    ...
    for (int i = 0; i < nrays; i++) {
      for (int j = 0; j < nrays; j++) {
        RNScalar x = image->Width() * (i+0.5) / nrays;
        RNScalar y = image->Height() * (j+0.5) / nrays;
        ...
        if (mesh->Intersection(ray, &intersection)) {
          R3Sphere(intersection.point, 0.25).Draw();
        }
      }
    }
  }
...
static void 
DrawTriangles(int side, RNBoolean transform, int color_scheme)
{
  // Draw triangles
  glDisable(GL_LIGHTING);
  for (int ir = 0; ir < scene->NRuns(); ir++) {
    ...
    for (int is = 0; is < run->NSegments(); is++) {
      ...
      for (int ia = 0; ia < segment->NScans(); ia++) {
        ...
        if (scan_data->texture) {
          ...
          // Set texture transformation
          glMatrixMode(GL_TEXTURE);
          glScaled(1.0 / scan_data->texture->Image()->Width(), 1.0 / scan_data->texture->Image()->Height(), 1.0);
          glMatrixMode(GL_MODELVIEW);
        }

        // Draw triangles
        for (int i = 0; i < scan_data->position_x_grid.XResolution()-1; i++) {
          ...
          for (int j = 0; j < scan_data->position_x_grid.YResolution()-1; j++) {
            const RNLength max_depth_difference = 1;
            RNLength min_depth = -1;
            RNLength max_depth = -1;

            // Get first corner
            R3Point position00(0, 0, 0);
            GSVScanline *scanline00 = NULL;
            RNScalar x00 = scan_data->position_x_grid.GridValue(i, j);
            if (x00 != R2_GRID_UNKNOWN_VALUE) {
              RNScalar y00 = scan_data->position_y_grid.GridValue(i, j);
              if (y00 != R2_GRID_UNKNOWN_VALUE) {
                RNScalar z00 = scan_data->position_z_grid.GridValue(i, j);
                if (z00 != R2_GRID_UNKNOWN_VALUE) {
                  RNScalar scanline_index_value = scan_data->scanline_grid.GridValue(i, j+1);
                  if (scanline_index_value != R2_GRID_UNKNOWN_VALUE) {
                    int scanline_index = (int) (scanline_index_value + 0.5);
                    GSVScanline *scanline = scan->Scanline(scanline_index);
                    R3Point position(x00, y00, z00);
                    const GSVPose& pose = scanline->Pose();
                    RNLength depth = pose.Towards().Dot(position - pose.Viewpoint());
                    if ((min_depth < 0) || (depth > min_depth)) {
                      if ((max_depth < 0) || (depth < max_depth)) {
                        if (min_depth < 0) min_depth = depth - max_depth_difference;
                        if (max_depth < 0) max_depth = depth + max_depth_difference;
                        scanline00 = scanline;
                        position00 = position;
                        if (transform) {
                          const R3Affine& scanline_transformation = optimization->OptimizedTransformation(scanline);
                          position00.Transform(scanline_transformation);
                        }
                      }
                    }
                  }
            ...
            // Get second corner
            R3Point position01(0, 0, 0);
            GSVScanline *scanline01 = NULL;
            RNScalar x01 = scan_data->position_x_grid.GridValue(i, j+1);
            if (x01 != R2_GRID_UNKNOWN_VALUE) {
              RNScalar y01 = scan_data->position_y_grid.GridValue(i, j+1);
              if (y01 != R2_GRID_UNKNOWN_VALUE) {
                RNScalar z01 = scan_data->position_z_grid.GridValue(i, j+1);
                if (z01 != R2_GRID_UNKNOWN_VALUE) {
                  RNScalar scanline_index_value = scan_data->scanline_grid.GridValue(i, j+1);
                  if (scanline_index_value != R2_GRID_UNKNOWN_VALUE) {
                    int scanline_index = (int) (scanline_index_value + 0.5);
                    GSVScanline *scanline = scan->Scanline(scanline_index);
                    R3Point position(x01, y01, z01);
                    const GSVPose& pose = scanline->Pose();
                    RNLength depth = pose.Towards().Dot(position - pose.Viewpoint());
                    if ((min_depth < 0) || (depth > min_depth)) {
                      if ((max_depth < 0) || (depth < max_depth)) {
                        ...
                        if (transform) {
                          const R3Affine& scanline_transformation = optimization->OptimizedTransformation(scanline);
                          position01.Transform(scanline_transformation);
                        }
                      }
                    }
                  }
            ...
            // Get third corner
            R3Point position10(0, 0, 0);
            GSVScanline *scanline10 = NULL;
            RNScalar x10 = scan_data->position_x_grid.GridValue(i+1, j);
            if (x10 != R2_GRID_UNKNOWN_VALUE) {
              RNScalar y10 = scan_data->position_y_grid.GridValue(i+1, j);
              if (y10 != R2_GRID_UNKNOWN_VALUE) {
                RNScalar z10 = scan_data->position_z_grid.GridValue(i+1, j);
                if (z10 != R2_GRID_UNKNOWN_VALUE) {
                  RNScalar scanline_index_value = scan_data->scanline_grid.GridValue(i, j+1);
                  if (scanline_index_value != R2_GRID_UNKNOWN_VALUE) {
                    int scanline_index = (int) (scanline_index_value + 0.5);
                    GSVScanline *scanline = scan->Scanline(scanline_index);
                    R3Point position(x10, y10, z10);
                    const GSVPose& pose = scanline->Pose();
                    RNLength depth = pose.Towards().Dot(position - pose.Viewpoint());
                    if ((min_depth < 0) || (depth > min_depth)) {
                      if ((max_depth < 0) || (depth < max_depth)) {
                        ...
                        if (transform) {
                          const R3Affine& scanline_transformation = optimization->OptimizedTransformation(scanline);
                          position10.Transform(scanline_transformation);
                        }
                      }
                    }
                  }
            ...
            // Get fourth corner
            R3Point position11(0, 0, 0);
            GSVScanline *scanline11 = NULL;
            RNScalar x11 = scan_data->position_x_grid.GridValue(i+1, j+1);
            if (x11 != R2_GRID_UNKNOWN_VALUE) {
              RNScalar y11 = scan_data->position_y_grid.GridValue(i+1, j+1);
              if (y11 != R2_GRID_UNKNOWN_VALUE) {
                RNScalar z11 = scan_data->position_z_grid.GridValue(i+1, j+1);
                if (z11 != R2_GRID_UNKNOWN_VALUE) {
                  RNScalar scanline_index_value = scan_data->scanline_grid.GridValue(i, j+1);
                  if (scanline_index_value != R2_GRID_UNKNOWN_VALUE) {
                    int scanline_index = (int) (scanline_index_value + 0.5);
                    GSVScanline *scanline = scan->Scanline(scanline_index);
                    R3Point position(x11, y11, z11);
                    const GSVPose& pose = scanline->Pose();
                    RNLength depth = pose.Towards().Dot(position - pose.Viewpoint());
                    if ((min_depth < 0) || (depth > min_depth)) {
                      if ((max_depth < 0) || (depth < max_depth)) {
                        ...
                        if (transform) {
                          const R3Affine& scanline_transformation = optimization->OptimizedTransformation(scanline);
                          position11.Transform(scanline_transformation);
                        }
                      }
                    }
                  }
              ...
              if (scanline10) {
                if (scanline01) {
                  if (scanline11) {
                    // Draw quad
                    glBegin(GL_QUADS);  
                    if (scan_data->texture) glTexCoord2i(i, j);
                    R3LoadPoint(position00);
                    if (scan_data->texture) glTexCoord2i(i+1, j);
                    R3LoadPoint(position10);
                    if (scan_data->texture) glTexCoord2i(i+1, j+1);
                    R3LoadPoint(position11);
                    if (scan_data->texture) glTexCoord2i(i+1, j);
                    R3LoadPoint(position01);
                    glEnd();
                  }
                  else {
                    // Draw triangle
...
void AtExit(void)
{
  ...
  // Write feature grids
  if (output_feature_grid_directory || output_feature_grid_name) {
    if (!WriteFeatureGrids(optimization, output_feature_grid_directory, output_feature_grid_name)) exit(-1);
  }
...
void GLUTRedraw(void)
{
  ...
  for (int side = 0; side <= split_screen; side++) {
   ...
   // Draw pairs
    ...
    // Draw triangles
    if (show_triangles[side]) {
      DrawTriangles(side, show_optimization[side], color_scheme[side]);
    }
...
void GLUTKeyboard(unsigned char key, int x, int y)
{
  ...
  case '%':
    show_triangles[side] = !show_triangles[side];
    break;
        ...
        if (selected_feature[side]) {
          for (int i = 0; i < optimization->NPairs(); i++) {
            ...
            printf("%d %d : %g %g %g : %g %g\n", feature0->index, feature1->index, 
                   ...
                   optimization->PairScore(feature0, feature1),
...
ParseArgs(int argc, char **argv)
{
  ...
  while (argc > 0) {
    if ((*argv)[0] == '-') {
      ...
      else if (!strcmp(*argv, "-output_feature_grid_directory")) { argc--; argv++; output_feature_grid_directory = *argv; }
      else if (!strcmp(*argv, "-output_feature_grid_name")) { argc--; argv++; output_feature_grid_name = *argv; }
      ...
      else if (!strcmp(*argv, "-create_mesh_correspondences")) { create_correspondences = create_mesh_correspondences = 1; }
      ...
      else if (!strcmp(*argv, "-load_textures")) { load_textures = 1; }
  ...
  if (!output_scene_name && !output_correspondences_name && !output_transformations_name && !output_inertias_name && 
      !output_feature_grid_directory && !output_feature_grid_name && 
