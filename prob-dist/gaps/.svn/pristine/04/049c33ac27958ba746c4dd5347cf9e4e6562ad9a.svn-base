...
const char *base_image_names [] = {
  "Scanline", "PointIndex", "Timestamp", "TravelDistance", 
...
const char *geodesic_image_names [] = {
  "GeodesicGroundDistance", "GeodesicGroundIndex"
};
...
const char *label_image_names [] = {
  "CurbLabelDepth", "BaselineLabelDepth", "MajorStructureLabel", 
};
...
int num_geodesic_image_names = sizeof(geodesic_image_names) / sizeof(const char *);
int num_color_image_names = sizeof(color_image_names) / sizeof(const char *);
int num_label_image_names = sizeof(label_image_names) / sizeof(const char *);
...
WriteNormalImages(GSVScan *scan, const char *output_image_directory, const char *image_parameterization)
{
  ...
  if (!overwrite_existing_files && RNFileExists(image_name)) return 1;
...
static int
WriteGeodesicImages(GSVScan *scan, const char *output_image_directory, const char *image_parameterization,
  RNLength max_ground_height = 0.25, RNLength max_distance_gap = 0.5, int max_index_gap = 8)
{
  // Get convenient variables
  if (!scan) return 0;
  if (scan->NScanlines() == 0) return 1;
  ...
  // Skip if already done
  char image_name[4096];
  sprintf(image_name, "%s/%s/%02d_%02d_%s_GeodesicGroundDistance.grd", output_image_directory, run->Name(), segment_index, scan_index, image_parameterization);
  if (!overwrite_existing_files && RNFileExists(image_name)) return 1;
  
  // Print message
  if (print_debug) {
    printf("    Creating geodesic images ...\n");
    fflush(stdout);
  }

  // Read grids
  R2Grid height_grid, positionX_grid, positionY_grid, positionZ_grid;
  sprintf(image_name, "%s/%s/%02d_%02d_%s_Height.grd", output_image_directory, run->Name(), segment_index, scan_index, image_parameterization);
  ...
  // Create geodesic grids
  R2Grid geodesic_ground_distance_grid(positionX_grid);   
  geodesic_ground_distance_grid.Clear(R2_GRID_UNKNOWN_VALUE);
  R2Grid geodesic_ground_index_grid(geodesic_ground_distance_grid);   
  R2Grid geodesic_ground_mask(geodesic_ground_distance_grid);   

  int niterations = height_grid.Maximum() / max_ground_height + 1;
  for (int iteration = 1; iteration <= niterations; iteration++) {
    double height_threshold = iteration * max_ground_height; 
    double gap_threshold = iteration * max_distance_gap;
    RNBoolean done = TRUE;

    // Create heap containing seed points
    RNHeap<const RNScalar *> heap(0);
    const RNScalar *geodesic_ground_distance_grid_valuesp = geodesic_ground_distance_grid.GridValues();
    for (int i = 0; i < geodesic_ground_distance_grid.NEntries(); i++) {
      // Check position values
      RNScalar x = positionX_grid.GridValue(i);
      if (x == R2_GRID_UNKNOWN_VALUE) continue;
      RNScalar y = positionY_grid.GridValue(i);
      if (y == R2_GRID_UNKNOWN_VALUE) continue;
      RNScalar z = positionZ_grid.GridValue(i);
      if (z == R2_GRID_UNKNOWN_VALUE) continue;
      RNScalar cm = geodesic_ground_mask.GridValue(i);
      if (cm != R2_GRID_UNKNOWN_VALUE) continue;
      done = FALSE;

      // Check height value
      RNScalar height = height_grid.GridValue(i);
      if (height == R2_GRID_UNKNOWN_VALUE) continue;
      if (height > height_threshold) continue;

      // Update grids
      geodesic_ground_distance_grid.SetGridValue(i, height);
      geodesic_ground_index_grid.SetGridValue(i, i);

      // Update heap
      const RNScalar *geodesic_ground_distance_grid_valuep = geodesic_ground_distance_grid_valuesp + i;
      heap.Push(geodesic_ground_distance_grid_valuep);
    }
    ...
    while (!heap.IsEmpty()) {
      int ci, cj;
      const RNScalar *geodesic_ground_distance_grid_valuep = heap.Pop();
      int grid_index = geodesic_ground_distance_grid_valuep - geodesic_ground_distance_grid_valuesp;
      geodesic_ground_distance_grid.IndexToIndices(grid_index, ci, cj);

      // Get grid values
      RNScalar cx = positionX_grid.GridValue(ci, cj);
      RNScalar cy = positionY_grid.GridValue(ci, cj);
      RNScalar cz = positionZ_grid.GridValue(ci, cj);
      RNScalar cd = geodesic_ground_distance_grid.GridValue(ci, cj);
      RNScalar ck = geodesic_ground_index_grid.GridValue(ci, cj);
      R3Point cp(cx, cy, cz);

      // Just checking
      assert(cx != R2_GRID_UNKNOWN_VALUE);
      assert(cy != R2_GRID_UNKNOWN_VALUE);
      assert(cz != R2_GRID_UNKNOWN_VALUE);
      assert(cd != R2_GRID_UNKNOWN_VALUE);
      assert(ck != R2_GRID_UNKNOWN_VALUE);

      // Consider neighbors
      for (int s = -max_index_gap; s <= max_index_gap; s++) {
        int ni = ci + s;
        if ((ni < 0) || (ni >= geodesic_ground_distance_grid.XResolution())) continue;
        for (int t = -max_index_gap; t <= max_index_gap; t++) {
          int nj = cj + t;
          if ((nj < 0) || (nj >= geodesic_ground_distance_grid.YResolution())) continue;

          // Get neighbor index info
          int neighbor_index = 0;
          geodesic_ground_distance_grid.IndicesToIndex(ni, nj, neighbor_index);
          const RNScalar *neighbor_geodesic_ground_distance_grid_valuep = geodesic_ground_distance_grid_valuesp + neighbor_index;

          // Get neighbor grid values
          RNScalar nx = positionX_grid.GridValue(ni, nj);
          if (nx == R2_GRID_UNKNOWN_VALUE) continue;
          RNScalar ny = positionY_grid.GridValue(ni, nj);
          if (ny == R2_GRID_UNKNOWN_VALUE) continue;
          RNScalar nz = positionZ_grid.GridValue(ni, nj);
          if (nz == R2_GRID_UNKNOWN_VALUE) continue;
          RNScalar nh = height_grid.GridValue(ni, nj);
          if (nh == R2_GRID_UNKNOWN_VALUE) continue;
          if (nh <= height_threshold) continue;
          RNScalar nd = geodesic_ground_distance_grid.GridValue(ni, nj);
          RNScalar nm = geodesic_ground_mask.GridValue(ni, nj);
          if (nm != R2_GRID_UNKNOWN_VALUE) continue;
          R3Point np(nx, ny, nz);

          // Check gap 
          RNScalar gap = R3Distance(cp, np);
          ...
          if (nd == R2_GRID_UNKNOWN_VALUE) {
            geodesic_ground_distance_grid.SetGridValue(ni, nj, cd + gap);
            geodesic_ground_index_grid.SetGridValue(ni, nj, ck);
            heap.Push(neighbor_geodesic_ground_distance_grid_valuep);
          }
          else if (nd > cd + gap) {
            geodesic_ground_distance_grid.SetGridValue(ni, nj, cd + gap);
            geodesic_ground_index_grid.SetGridValue(ni, nj, ck);
            heap.Update(neighbor_geodesic_ground_distance_grid_valuep);
          }
        }
      }
    }
    ...
    // Update mask
    geodesic_ground_mask = geodesic_ground_distance_grid;
  }

  // Write grids
  sprintf(image_name, "%s/%s/%02d_%02d_%s_GeodesicGroundDistance.grd", output_image_directory, run->Name(), segment_index, scan_index, image_parameterization);
  geodesic_ground_distance_grid.Write(image_name);
  sprintf(image_name, "%s/%s/%02d_%02d_%s_GeodesicGroundIndex.grd", output_image_directory, run->Name(), segment_index, scan_index, image_parameterization);
  geodesic_ground_index_grid.Write(image_name);
...
WriteColorImages(GSVScan *scan, const char *output_image_directory, const char *image_parameterization)
{
  ...
  int xres = (int) (color_zoom_factor * scanline_grid.XResolution());
  int yres = (int) (color_zoom_factor * scanline_grid.YResolution());
  ...
  world_to_grid.Scale(color_zoom_factor);
  ...
  int ntapestries[3] = { 5, 3, 5 };
  int tapestries[3][5] = { { 0, 4, 5, 6, 7 }, { 0, 1, 7 }, { 0, 1, 2, 3, 4 } };
  for (int k = 0; k < ntapestries[scan_index]; k++) {
    ...
    for (int ii = 0; ii < tapestry->NImages(); ii++) {
      ...
      if (!undistorted_image) continue;

      // Get depth image
      R2Grid depth_image;
      sprintf(image_name, "%s/%s/%02d_%06d_%02d_UV_Depth.pfm", output_image_directory, run->Name(), segment_index, panorama_index, image_index);
      if (!depth_image.Read(image_name)) return 0;
      RNScalar depth_image_scale_factor = (RNScalar) depth_image.XResolution() / (RNScalar) undistorted_image->Width();

      // Consider every grid pixel
      for (int ix = 0; ix < xres; ix++) {
        for (int iy = 0; iy < yres; iy++) {
          RNCoord px = ix / color_zoom_factor;
          int ipx = (int) (px + 0.5);
          if (ipx >= position_x_grid.XResolution()) continue;
          RNCoord py = iy / color_zoom_factor;
          int ipy = (int) (py + 0.5);
          if (ipy >= position_x_grid.YResolution()) continue;
          ...
          if (z == R2_GRID_UNKNOWN_VALUE) continue;
          RNScalar scanline_value = scanline_grid.GridValue(ipx, ipy);
          ...
          if ((image_ix1 < 0) || (image_ix1 >= undistorted_image->Width())) continue;
          if ((image_iy1 < 0) || (image_iy1 >= undistorted_image->Height())) continue;
          int image_ix2 = image_ix1 + 1;
          int image_iy2 = image_iy1 + 1;
          if ((image_ix2 < 0) || (image_ix2 >= undistorted_image->Width())) continue;
          if ((image_iy2 < 0) || (image_iy2 >= undistorted_image->Height())) continue;

          // Check whether point is occluded
          const RNLength visibility_tolerance = 1.0;
          RNLength image_depth = depth_image.GridValue(image_position * depth_image_scale_factor);
          R3Vector vector = world_position - image_viewpoint;
          RNScalar point_depth = vector.Dot(image_pose.Towards());
          if (image_depth < point_depth - visibility_tolerance) continue;

          // Compute whether point is occluded
          if (mesh) {
            R3MeshIntersection intersection;
            R3Ray ray(image_viewpoint, world_position);
            // if (mesh->Intersection(ray, &intersection, min_frustum_depth, max_frustum_depth)) {
            if (mesh->Intersection(ray, &intersection)) {
              if (intersection.t < point_depth - visibility_tolerance) continue;
            }
          }

          // Retrieve color of point from undistorted image
          RNRgb color11 = undistorted_image->PixelRGB(image_ix1, image_iy1);
          RNRgb color12 = undistorted_image->PixelRGB(image_ix1, image_iy2);
          RNRgb color21 = undistorted_image->PixelRGB(image_ix2, image_iy1);
          RNRgb color22 = undistorted_image->PixelRGB(image_ix2, image_iy2);
    ...
    for (int ix = 0; ix < xres; ix++) {
      for (int iy = 0; iy < yres; iy++) {
        tapestry_rgb_image.SetPixelRGB(ix, iy, unknown_color);
  ...
  for (int ix = 0; ix < xres; ix++) {
    for (int iy = 0; iy < yres; iy++) {
      scan_rgb_image.SetPixelRGB(ix, iy, unknown_color);
...
WriteXYImages(GSVSegment *segment, const char *output_image_directory)
{
  ...
  if (include_base_images) {
    for (int i = 0; i < num_xy_image_names; i++) {
      ...
      if (!overwrite_existing_files && RNFileExists(image_name)) continue;
...
WriteSAImages(GSVScan *scan, const char *output_image_directory)
{
  ...
  if (include_geodesic_images) {
    if (!WriteGeodesicImages(scan, output_image_directory, "SA")) return 0;
  }
  ...
  const char *grid_name)
{
  ...
  if (!overwrite_existing_files && RNFileExists(da_name)) return 1;
...
WriteDAImages(GSVScan *scan, const char *output_image_directory)
{
  ...
  if (include_geodesic_images) {
    if (!WriteGeodesicImages(scan, output_image_directory, "DA")) return 0;
  }
  if (include_color_images) {
    if (!WriteColorImages(scan, output_image_directory, "DA")) return 0;
  }
#else
  if (include_normal_images) {
  ...
  if (include_geodesic_images) {
    for (int i = 0; i < num_geodesic_image_names; i++) {
      if (!WriteDAImage(scan, output_image_directory, sa_travel_distance_grid, sa_viewpoint_distance_grid, geodesic_image_names[i])) return 0;
    }
  }
  if (include_color_images) {
    ...
    // Write bmp file
    char image_name[4096];
    sprintf(image_name, "%s/%s/%02d_%02d_DA_Color.bmp", output_image_directory, run->Name(), segment_index, scan_index);
    if (overwrite_existing_files || !RNFileExists(image_name)) {
      // Read red, green, and blue grids
      R2Grid red_grid, green_grid, blue_grid;
      sprintf(image_name, "%s/%s/%02d_%02d_DA_ColorRed.grd", output_image_directory, run->Name(), segment_index, scan_index);
      red_grid.Read(image_name);
      sprintf(image_name, "%s/%s/%02d_%02d_DA_ColorGreen.grd", output_image_directory, run->Name(), segment_index, scan_index);
      green_grid.Read(image_name);
      sprintf(image_name, "%s/%s/%02d_%02d_DA_ColorBlue.grd", output_image_directory, run->Name(), segment_index, scan_index);
      blue_grid.Read(image_name);

      // Create RGB image
      R2Image image(red_grid.XResolution(), red_grid.YResolution(), 3);
      for (int j = 0; j < image.Height(); j++) {
        for (int i = 0; i < image.Width(); i++) {
          image.SetPixelRGB(i, j, unknown_color);
          RNScalar r = red_grid.GridValue(i, j);
          if (r == R2_GRID_UNKNOWN_VALUE) continue;
          RNScalar g = green_grid.GridValue(i, j);
          if (g == R2_GRID_UNKNOWN_VALUE) continue;
          RNScalar b = blue_grid.GridValue(i, j);
          if (b == R2_GRID_UNKNOWN_VALUE) continue;
  ...
  const char *grid_name)
{
  ...
  if (!overwrite_existing_files && RNFileExists(dh_name)) return 1;
...
WriteDHImages(GSVScan *scan, const char *output_image_directory)
{
  ...
  if (include_geodesic_images) {
    for (int i = 0; i < num_geodesic_image_names; i++) {
      if (!WriteDHImage(scan, output_image_directory, da_distance_grid, da_height_grid, geodesic_image_names[i])) return 0;
    }
  }
  if (include_color_images) {
    for (int i = 0; i < num_color_image_names; i++) {
    ...
    // Write bmp file
    char image_name[4096];
    sprintf(image_name, "%s/%s/%02d_%02d_DH_Color.bmp", output_image_directory, run->Name(), segment_index, scan_index);
    if (overwrite_existing_files || !RNFileExists(image_name)) {
      // Read red, green, and blue grids
      R2Grid red_grid, green_grid, blue_grid;
      sprintf(image_name, "%s/%s/%02d_%02d_DH_ColorRed.grd", output_image_directory, run->Name(), segment_index, scan_index);
      red_grid.Read(image_name);
      sprintf(image_name, "%s/%s/%02d_%02d_DH_ColorGreen.grd", output_image_directory, run->Name(), segment_index, scan_index);
      green_grid.Read(image_name);
      sprintf(image_name, "%s/%s/%02d_%02d_DH_ColorBlue.grd", output_image_directory, run->Name(), segment_index, scan_index);
      blue_grid.Read(image_name);
  ...
  if (include_label_images) {
    for (int i = 0; i < num_label_image_names; i++) {
      if (!WriteDHImage(scan, output_image_directory, da_distance_grid, da_height_grid, label_image_names[i])) return 0;
    }
  }
...
static void
RasterizeTriangle(R2Grid& depth_grid, R2Grid& s_index_grid, R2Grid& a_index_grid,
  const R2Point position[3], RNScalar depth[3], int s_index[3], int a_index[3], RNScalar scale_factor = 1.0)
{
  ...
  double y0 = scale_factor * position[iv0].Y();
  double y1 = scale_factor * position[iv1].Y();
  double y2 = scale_factor * position[iv2].Y();
  double x0 = scale_factor * position[iv0].X();
  double x2 = scale_factor * position[iv2].X();
  double x1a = scale_factor * position[iv1].X();
  ...
  double s_index0 = s_index[iv0];
  double s_index2 = s_index[iv2];
  double s_index1a = s_index[iv1];
  double s_index1b = (1-t1)*s_index0 + t1*s_index2;
  double a_index0 = a_index[iv0];
  double a_index2 = a_index[iv2];
  double a_index1a = a_index[iv1];
  double a_index1b = (1-t1)*a_index0 + t1*a_index2;
  if (x1a > x1b) { 
    double swap;
    swap = x1a; x1a = x1b; x1b = swap; 
    swap = depth1a; depth1a = depth1b; depth1b = swap; 
    swap = s_index1a; s_index1a = s_index1b; s_index1b = swap; 
    swap = a_index1a; a_index1a = a_index1b; a_index1b = swap; 
  }
  ...
  double s_indexa_step = (s_index1a  - s_index0) / nysteps;
  double s_indexb_step = (s_index1b  - s_index0) / nysteps;
  double a_indexa_step = (a_index1a  - a_index0) / nysteps;
  double a_indexb_step = (a_index1b  - a_index0) / nysteps;
  double xa = x0;
  double xb = x0;
  double deptha = depth0;
  double depthb = depth0;
  double s_indexa = s_index0;
  double s_indexb = s_index0;
  double a_indexa = a_index0;
  double a_indexb = a_index0;
  for (int iy = iy0; iy <= iy1; iy++) {
    if ((iy >= 0) && (iy < depth_grid.YResolution())) {
      ...
      double s_index_step = (s_indexb - s_indexa) / nxsteps;
      double a_index_step = (a_indexb - a_indexa) / nxsteps;
      double depth = deptha;
      double s_index = s_indexa;
      double a_index = a_indexa;
      for (int ix = ixa; ix <= ixb; ix++) {
        if ((ix >= 0) && (ix < depth_grid.XResolution())) {
          if (depth < depth_grid.GridValue(ix, iy)) {
            depth_grid.SetGridValue(ix, iy, depth);
            s_index_grid.SetGridValue(ix, iy, s_index);
            a_index_grid.SetGridValue(ix, iy, a_index);
          }
        }
        depth += depth_step;
        s_index += s_index_step;
        a_index += a_index_step;
      }
    }
    xa += xa_step;
    xb += xb_step;
    deptha += deptha_step;
    depthb += depthb_step;
    s_indexa += s_indexa_step;
    s_indexb += s_indexb_step;
    a_indexa += a_indexa_step;
    a_indexb += a_indexb_step;
  }
  ...
  s_indexa_step = (s_index2  - s_index1a) / nysteps;
  s_indexb_step = (s_index2  - s_index1b) / nysteps;
  a_indexa_step = (a_index2  - a_index1a) / nysteps;
  a_indexb_step = (a_index2  - a_index1b) / nysteps;
  xa = x1a;
  xb = x1b;
  deptha = depth1a;
  depthb = depth1b;
...
static int
WriteUVDepthImages(GSVImage *image, const char *output_image_directory, GSVMesh **scan_meshes = NULL, RNBoolean fill_holes = FALSE)
{
  // Get convenient variables
  ...
  // Get filenames 
  char depth_grid_name[4096], scanline_index_grid_name[4096], point_index_grid_name[4096];
  sprintf(depth_grid_name, "%s/%s/%02d_%06d_%02d_UV_Depth.pfm", 
    output_image_directory, run->Name(), segment_index, panorama_index, image_index);
  sprintf(scanline_index_grid_name, "%s/%s/%02d_%06d_%02d_UV_Scanline.pfm", 
    output_image_directory, run->Name(), segment_index, panorama_index, image_index);
  sprintf(point_index_grid_name, "%s/%s/%02d_%06d_%02d_UV_PointIndex.pfm", 
    output_image_directory, run->Name(), segment_index, panorama_index, image_index);

  // Skip if already done
  if (!overwrite_existing_files && RNFileExists(depth_grid_name)) return 1;
  ...
  R2Box image_bbox(0, 0, image->Width()-1, image->Height()-1);
  const GSVPose& pose = image->Pose();
  R3Frustum frustum(pose.Viewpoint(), pose.Towards(), pose.Up(), 
    camera->XFov(), camera->YFov(), min_frustum_depth, max_frustum_depth);

  // Create grids
  int xres = (int) (UV_scale_factor * image->Width() + 0.5);
  int yres = (int) (UV_scale_factor * image->Height() + 0.5);
  RNScalar scale_factor = (double) xres / (double) image->Width();
  R2Affine pixels_to_grid = R2identity_affine;
  pixels_to_grid.Scale(scale_factor, scale_factor);
  R2Grid depth_grid(xres, yres, pixels_to_grid);
  R2Grid scanline_index_grid(xres, yres, pixels_to_grid);
  R2Grid point_index_grid(xres, yres, pixels_to_grid);
  depth_grid.Clear(max_frustum_depth);
  scanline_index_grid.Clear(R2_GRID_UNKNOWN_VALUE);
  point_index_grid.Clear(R2_GRID_UNKNOWN_VALUE);
  ...
  for (int ia = 0; ia < segment->NScans(); ia++) {
    ...
    int scanline_index[3] = { -1, -1, -1 };
    int point_index[3] = { -1, -1, -1 };

    // Get mesh
    GSVMesh *mesh = scan_meshes[ia];
    if (!mesh) continue;

    // Rasterize faces
    for (int i = 0; i < mesh->NFaces(); i++) {
      GSVMeshFace *face = mesh->Face(i);
      int count = 0;
      for (int j = 0; j < 3; j++) {
        GSVMeshVertex *vertex = mesh->VertexOnFace(face, j);
        GSVScanline *scanline = mesh->VertexScanline(vertex);
        if (!scanline) continue;
        R3Point world_position = mesh->VertexPosition(vertex);
        if (!frustum.Intersects(world_position)) continue;
        undistorted_position[count] = image->UndistortedPosition(world_position);
        if (undistorted_position[count].X() < 0) continue;
        if (undistorted_position[count].Y() < 0) continue;
        if (!R2Contains(image_bbox, undistorted_position[count])) continue;
        R2Point distorted_position = image->DistortedPosition(undistorted_position[count]);
        if (distorted_position.X() < 0) continue;
        if (distorted_position.Y() < 0) continue;
        ...
        depth[count] = vector.Dot(towards);
        scanline_index[count] = scanline->SegmentIndex();
        point_index[count] = mesh->VertexPointIndex(vertex);
        count++;
      }
      if (count < 1) continue;
      if (count < 2) { undistorted_position[1] = undistorted_position[0]; depth[1] = depth[0]; scanline_index[1] = scanline_index[0]; point_index[1] = point_index[0]; }
      if (count < 3) { undistorted_position[2] = undistorted_position[1];  depth[2] = depth[1]; scanline_index[2] = scanline_index[1]; point_index[2] = point_index[1]; }
      RasterizeTriangle(depth_grid, scanline_index_grid, point_index_grid, undistorted_position, depth, scanline_index, point_index, UV_scale_factor);
    }

    // Rasterize edges
    for (int i = 0; i < mesh->NEdges(); i++) {
      GSVMeshEdge *edge = mesh->Edge(i);
      if (mesh->FaceOnEdge(edge)) continue;
      int count = 0;
      for (int j = 0; j < 2; j++) {
        GSVMeshVertex *vertex = mesh->VertexOnEdge(edge, j);
        GSVScanline *scanline = mesh->VertexScanline(vertex);
        if (!scanline) continue;
        R3Point world_position = mesh->VertexPosition(vertex);
        if (!frustum.Intersects(world_position)) continue;
    ...
    for (int i = 0; i < mesh->NVertices(); i++) {
      ...
      if (mesh->EdgeOnVertex(vertex)) continue;
      GSVScanline *scanline = mesh->VertexScanline(vertex);
      if (!scanline) continue;
      R3Point world_position = mesh->VertexPosition(vertex);
      if (!frustum.Intersects(world_position)) continue;
      undistorted_position[0] = image->UndistortedPosition(world_position);
      if (undistorted_position[0].X() < 0) continue;
      if (undistorted_position[0].Y() < 0) continue;
      if (!R2Contains(image_bbox, undistorted_position[0])) continue;
      R2Point distorted_position = image->DistortedPosition(undistorted_position[0]);
      if (distorted_position.X() < 0) continue;
      if (distorted_position.Y() < 0) continue;
      if (!R2Contains(image_bbox, distorted_position)) continue;
      int column_index = (int) (distorted_position.X() + 0.5);
      ...
      const R3Vector& towards = pose.Towards();
      R3Vector vector = world_position - viewpoint;
      depth[0] = vector.Dot(towards);
      scanline_index[0] = scanline->SegmentIndex();
      point_index[0] = mesh->VertexPointIndex(vertex);
      undistorted_position[2] = undistorted_position[1] = undistorted_position[0]; 
      depth[2] = depth[1] = depth[0];
      scanline_index[2] = scanline_index[1] = scanline_index[0];
  ...
  scanline_index_grid.Write(scanline_index_grid_name);
...
static int
WriteUVDepthImages(GSVSegment *segment, const char *output_image_directory, RNBoolean fill_holes = FALSE)
{
  // Read scan meshes
  GSVMesh **scan_meshes = new GSVMesh * [ segment->NScans() ];
  for (int ia = 0; ia < segment->NScans(); ia++) {
    GSVScan *scan = segment->Scan(ia);
    if (ia == 1) scan_meshes[ia] = NULL;
    else scan_meshes[ia] = scan->Mesh();
  }

  // Write UV depth images
  for (int ip = 0; ip < segment->NPanoramas(); ip++) {
    ...
    for (int ii = 0; ii < panorama->NImages(); ii++) {
      ...
      if (!WriteUVDepthImages(image, output_image_directory, scan_meshes, fill_holes)) return 0;
    }
  }

  // Release scan meshes
  for (int ia = 0; ia < segment->NScans(); ia++) 
    if (scan_meshes[ia]) delete scan_meshes[ia];
  delete [] scan_meshes;
...
WriteUVImage(GSVImage *image, const char *output_image_directory, const char *grid_name)
{
  ...
  // Skip if already done
  if (!overwrite_existing_files && RNFileExists(output_grid_name)) return 1;
  ...
  sprintf(image_name, "%s/%s/%02d_%06d_%02d_UV_Scanline.pfm", 
    output_image_directory, run->Name(), segment_index, panorama_index, image_index);
  if (!scanline_index_grid.Read(image_name)) return 0;
  sprintf(image_name, "%s/%s/%02d_%06d_%02d_UV_PointIndex.pfm", 
    output_image_directory, run->Name(), segment_index, panorama_index, image_index);
  ...
  // Create output grid
  R2Grid output_grid(scanline_index_grid);
  output_grid.Clear(R2_GRID_UNKNOWN_VALUE);

  // Create output grid
  for (int iy = 0; iy < scanline_index_grid.YResolution(); iy++) {
    for (int ix = 0; ix < scanline_index_grid.XResolution(); ix++) {
      RNScalar scanline_index_value = scanline_index_grid.GridValue(ix, iy);
      if (scanline_index_value == R2_GRID_UNKNOWN_VALUE) continue;
      int scanline_index = (int) (scanline_index_value + 0.5);
      RNScalar point_index_value = point_index_grid.GridValue(ix, iy);
      if (point_index_value == R2_GRID_UNKNOWN_VALUE) continue;
      // int point_index = (int) (point_index_value + 0.5);
      GSVScanline *scanline = segment->Scanline(scanline_index);
      GSVScan *scan = scanline->Scan();
      int scan_index = scan->SegmentIndex();

      // Get value from SA grid
      int s = 0;
      int a = 0;
      RNAbort("Not implemented");
      RNScalar value = sa_grids[scan_index].GridValue(s, a);
      if (value == R2_GRID_UNKNOWN_VALUE) continue;
...
static int
WriteUVImages(GSVSegment *segment, const char *output_image_directory)
{
  // Write depth images 
  if (include_depth_images) {
    if (!WriteUVDepthImages(segment, output_image_directory, FALSE)) return 0;
  }
...
WriteImages(GSVScene *scene, const char *output_image_directory)
{
  ...
  for (int ir = 0; ir < scene->NRuns(); ir++) {
    ...
    for (int is = 0; is < run->NSegments(); is++) {
      ...
      if (capture_UV_images && !WriteUVImages(segment, output_image_directory)) return 0;
      for (int ia = 0; ia < segment->NScans(); ia++) {
        GSVScan *scan = segment->Scan(ia);
...
ParseArgs(int argc, char **argv)
{
  ...
  while (argc > 0) {
    if ((*argv)[0] == '-') {
      ...
      else if (!strcmp(*argv, "-overwrite")) { overwrite_existing_files = 1; }
      ...
      else if (!strcmp(*argv, "-geodesic")) { include_geodesic_images = 1; default_images = 0; }
      ...
      else if (!strcmp(*argv, "-color_zoom")) { argc--; argv++; color_zoom_factor = atof(*argv); }
