  ...
  R3SurfelNode *parent_node1)
{
 ...
 // COPY FEATURES
  ...
  for (int i = 0; i < scene2.NScans(); i++) {
    ...
    scan1->SetResolution(scan2->Width(), scan2->Height());
...
ReadAsciiFile(const char *filename)
{
  ...
  for (int i = 0; i < nscans; i++) {
    ...
    double px, py, pz, tx, ty, tz, ux, uy, uz, focal_length, timestamp;
    fscanf(fp, "%s", buffer);
    if (strcmp(buffer, "S")) { fprintf(stderr, "Error reading scan %d in %s\n", i, filename); return 0; }
    ReadAsciiName(fp, scan_name); 
    fscanf(fp, "%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%d%d%d%lf", &px, &py, &pz, &tx, &ty, &tz, &ux, &uy, &uz, &timestamp, &node_index, &width, &height, &focal_length);
    for (int j = 0; j < 5; j++) fscanf(fp, "%s", buffer);
    ...
    scan->SetResolution(width, height);
...
WriteAsciiFile(const char *filename) 
{
  ...
  for (int i = 0; i < NScans(); i++) {
    ...
    fprintf(fp, " %d %d %g", scan->Width(), scan->Height(), scan->FocalLength());
    for (int j = 0; j < 5; j++) fprintf(fp, " 0");
    fprintf(fp, "\n");
  }
...
WriteBinaryFile(const char *filename) 
{
  ...
  for (int i = 0; i < NScans(); i++) {
    ...
    WriteBinaryInteger(fp, scan->Width());
    WriteBinaryInteger(fp, scan->Height());
