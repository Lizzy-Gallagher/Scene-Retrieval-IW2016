  ...
  RNLength max_depth_discontinuity)
{
  ...
  if (!gsv_vertex_block) { 
    fprintf(stderr, "Unable to allocate GSV Mesh vertex array\n"); 
    scan->ReleasePoints(); 
    return 0; 
  }
  ...
  void *compatible_data) const
{
  ...
  if (!search_tree) {
    ...
    if (!search_tree) {
      fprintf(stderr, "Unable to build mesh search tree\n");
...
ReadPlyFile(GSVScan *scan, const char *filename)
{
  ...
  for (int i = 0; i < num_obj_infos; i++) {
    char obj_info[1024];
    strncpy(obj_info, obj_infos[i], 1024);
  ...
  for (i = 0; i < nelems; i++) {
    ...
    if (equal_strings ("vertex", elem_name)) {
      ...
      if (!gsv_vertex_block) {
        fprintf(stderr, "Unable to allocate vertex matrix\n");
        return 0;
      }
...
int GSVMesh::
WritePlyFile(GSVScan *scan, const char *filename, RNBoolean binary)
{
  typedef struct PlyVertex {
    float x, y, z;
    int scanline_index;
...
int GSVMesh::
WriteARFFFile(GSVScan *scan, const char *filename)
{
  // Open file
