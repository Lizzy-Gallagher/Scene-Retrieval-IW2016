...
GSVScanline *GSVScan::
FindScanlineBeforeTravelDistance(RNScalar traveldistance, int imin, int imax) const
{
  // Binary search
  int i = (imin + imax) / 2;
  if (i == imin) return Scanline(imin);
  assert(i < imax);
  GSVScanline *scanline = Scanline(i);
  RNScalar t = scanline->TravelDistance();
  if (t > traveldistance) return FindScanlineBeforeTravelDistance(traveldistance, imin, i);
  else if (t < traveldistance) return FindScanlineBeforeTravelDistance(traveldistance, i, imax);
  else return scanline;
}
...
GSVScanline *GSVScan::
FindScanlineBeforeTravelDistance(RNScalar traveldistance) const
{
  // Binary search
  if (NScanlines() == 0) return NULL;
  if (traveldistance <= Scanline(0)->TravelDistance()) return Scanline(0);
  if (traveldistance >= Scanline(NScanlines()-1)->TravelDistance()) return Scanline(NScanlines()-1);
  return FindScanlineBeforeTravelDistance(traveldistance, 0, NScanlines()-1);
}
