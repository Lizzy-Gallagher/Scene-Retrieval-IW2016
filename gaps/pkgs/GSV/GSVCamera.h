...
public:
  ...
  int UndistortImage(const R2Image& distorted_image, R2Image& output) const;
  int DistortImage(const R2Image& undistorted_image, R2Image& output) const;
...
public:
  ...
  R2Point DistortedPositionFromAngles(RNAngle x_dir, RNAngle y_dir) const;
...
ImageWidth(void) const
{
  ...
  // XXX THIS IS HARDWIRED FOR NOW XXX
  return 1936;
}
