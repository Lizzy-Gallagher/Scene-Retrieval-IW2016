...
PrintMemoryUsage(void)
{
  ...
  if (ret == 0) printf("Memory usage = %ld\n", usage.ru_maxrss);
  else printf("Cannot get memory usage\n");
}
...
static int 
IsScanlineCompatible(const R3Point& position, const R3Vector& normal,  R3Mesh *m2, R3MeshFace *f2, void *s1)
{
  // Get segment1
  GSVScanline *scanline1 = (GSVScanline *) s1;
  GSVScan *scan1 = scanline1->Scan();
  GSVSegment *segment1 = scan1->Segment();

  // Get segement2
  GSVMesh *mesh2 = (GSVMesh *) m2;
  GSVMeshVertex *vertex2 = mesh2->VertexOnFace(f2);
  GSVScanline *scanline2 = mesh2->VertexScanline(vertex2);
  GSVScan *scan2 = scanline2->Scan();
  GSVSegment *segment2 = scan2->Segment();
...
static R3MeshType
FindClosestVertex(GSVMesh *mesh, const R3Point& query, R3MeshIntersection *closest = NULL,  
  RNLength min_distance = 0, RNLength max_distance = RN_INFINITY, 
  ...
  void *compatible_data = NULL) 
{
  ...
  R3MeshType status = mesh->ClosestPoint(query, closest, min_distance, max_distance, IsCompatible, compatible_data);
  if (!status) return R3_MESH_NULL_TYPE;

  // Find closest vertex 
  if (closest) {
    if (closest->type == R3_MESH_EDGE_TYPE) {
      R3MeshVertex *vertexA = mesh->VertexOnEdge(closest->edge, 0);
      R3MeshVertex *vertexB = mesh->VertexOnEdge(closest->edge, 1);
      const R3Point& positionA = mesh->VertexPosition(vertexA);
      const R3Point& positionB = mesh->VertexPosition(vertexB);
      RNLength dA = R3Distance(positionA, query);
      RNLength dB = R3Distance(positionB, query);
      closest->vertex = (dA < dB) ? vertexA : vertexB;
    }
    else if (closest->type == R3_MESH_FACE_TYPE) {
      R3MeshVertex *vertexA = mesh->VertexOnFace(closest->face, 0);
      R3MeshVertex *vertexB = mesh->VertexOnFace(closest->face, 1);
      R3MeshVertex *vertexC = mesh->VertexOnFace(closest->face, 2);
      const R3Point& positionA = mesh->VertexPosition(vertexA);
      const R3Point& positionB = mesh->VertexPosition(vertexB);
      const R3Point& positionC = mesh->VertexPosition(vertexC);
      RNLength dA = R3Distance(positionA, query);
      RNLength dB = R3Distance(positionB, query);
      RNLength dC = R3Distance(positionC, query);
      if (dA < dB) {
        if (dA < dC) closest->vertex = vertexA;
        else closest->vertex = vertexC;
      }
      else {
        if (dB < dC) closest->vertex = vertexB;
        else closest->vertex = vertexC;
      }
    }
  }
...
int GSVPoseOptimization::
CreateMeshCorrespondence(GSVMesh *mesh1, GSVMesh *mesh20, GSVMesh *mesh22)
{
  // Pick a vertex at random from mesh1
  int vertex_index1 = (int) (RNRandomScalar() * mesh1->NVertices());
  GSVMeshVertex *vertex1 = mesh1->Vertex(vertex_index1);
  const R3Point& position1 = mesh1->VertexPosition(vertex1);
  GSVScanline *scanline1 = mesh1->VertexScanline(vertex1);
  if (!scanline1) return 0;
  int point_index1 = mesh1->VertexPointIndex(vertex1);
  if (point_index1 < 0) return 0;

  // Find the closest vertex
  GSVMesh *mesh2 = NULL;
  R3MeshIntersection closest2, closest20, closest22; closest2.t = FLT_MAX;
  if (mesh20) {
    if (FindClosestVertex(mesh20, position1, &closest20, 0, max_correspondence_world_distance, IsScanlineCompatible, scanline1)) {
      if (closest20.t < closest2.t) { mesh2 = mesh20; closest2 = closest20; }
    }
  }
  if (mesh22) {
    if (FindClosestVertex(mesh22, position1, &closest22, 0, max_correspondence_world_distance, IsScanlineCompatible, scanline1)) {
      if (closest22.t < closest2.t) { mesh2 = mesh22; closest2 = closest22; }
    }
  }

  // Get stuff for second vertex
  if (!mesh2) return 0;
  GSVMeshVertex *vertex2 = (GSVMeshVertex *) closest2.vertex;
  const R3Point& position2 = mesh2->VertexPosition(vertex2);
  GSVScanline *scanline2 = mesh2->VertexScanline(vertex2);
  if (!scanline2) return 0;
  int point_index2 = mesh2->VertexPointIndex(vertex2);
  if (point_index2 < 0) return 0;

  // Check if vertex1 and vertex2 are mutually closest
#if 0
  R3MeshIntersection closest1;
  if (mesh1->ClosestPoint(position2, &closest1, 0, max_correspondence_world_distance, IsScanlineCompatible, scanline2)) {
    if (!mesh1->IsVertexOnFace(vertex1, closest1.face)) return 0;
  }
#endif

  // Create feature1
  GSVFeature *feature1 = new GSVFeature(GSV_SCAN_POINT_FEATURE_TYPE, scanline1, point_index1, position1, R3zero_vector, R3zero_vector, 1, 1);
  InsertFeature(feature1);

  // Create feature2
  GSVFeature *feature2 = new GSVFeature(GSV_SCAN_POINT_FEATURE_TYPE, scanline2, point_index2, position2, R3zero_vector, R3zero_vector, 1, 1);
  InsertFeature(feature2);
...
int GSVPoseOptimization::
CreateMeshCorrespondences(GSVMesh *mesh1, GSVMesh *mesh20, GSVMesh *mesh22, int max_correspondences)
{
  // Create correspondences from mesh1 to mesh20 or mesh22
  int count = 0;
  for (int i = 0; i < max_correspondences; i++) {
    if (CreateMeshCorrespondence(mesh1, mesh20, mesh22)) count++;
    if (count >= max_correspondences) break;
  }

  // Return number of correspondences created
  return count;
}
...
int GSVPoseOptimization::
CreateMeshCorrespondences(RNScalar correspondences_per_cubic_meter)
{
  // Get first scan
  for (int ir1 = 0; ir1 < scene->NRuns(); ir1++) {
    GSVRun *run1 = scene->Run(ir1);
    for (int is1 = 0; is1 < run1->NSegments(); is1++) {
      GSVSegment *segment1 = run1->Segment(is1);
      for (int ia1 = 0; ia1 < segment1->NScans(); ia1++) {
        if (ia1 == 1) continue;
        GSVScan *scan1 = segment1->Scan(ia1);
        const R3Box& bbox1 = scan1->BBox();

        // Get second mesh
        GSVMesh *mesh1 = scan1->Mesh();
        if (!mesh1) continue;

        // Get second scan
        for (int ir2 = 0; ir2 < scene->NRuns(); ir2++) {
          GSVRun *run2 = scene->Run(ir2);
          for (int is2 = 0; is2 < run2->NSegments(); is2++) {
            GSVSegment *segment2 = run2->Segment(is2);
            if (segment2->NScans() < 3) continue;
            if (is2 == is1) continue;
            const R3Box& bbox2 = segment2->BBox();

            // Compute number of correspondences
            R3Box intersection = bbox1;
            intersection.Intersect(bbox2);
            if (intersection.IsEmpty()) continue;
            int num_correspondences = (int) (correspondences_per_cubic_meter * intersection.Volume());
            if (num_correspondences == 0) num_correspondences = 1;
            if (num_correspondences > 1024) num_correspondences = 1024;
            if (num_correspondences == 0) continue;

            // Get scans
            GSVScan *scan20 = segment2->Scan(0);
            GSVScan *scan22 = segment2->Scan(2);

            // Get bounding boxes
            const R3Box& bbox20 = scan20->BBox();
            const R3Box& bbox22 = scan22->BBox();

            // Get meshes
            GSVMesh *mesh20 = NULL;
            GSVMesh *mesh22 = NULL;
            if (R3Distance(bbox1, bbox20) < max_correspondence_world_distance) mesh20 = scan20->Mesh();
            if (R3Distance(bbox1, bbox22) < max_correspondence_world_distance) mesh22 = scan22->Mesh();

            // Create correspodnences 
            int count = CreateMeshCorrespondences(mesh1, mesh20, mesh22, num_correspondences);

            printf("MESH %d %d %d : %d %d : %d %d\n", ir1, is1, ia1, ir2, is2, num_correspondences, count);

            // Delete second mesh
            if (mesh20) delete mesh20;
            if (mesh22) delete mesh22;
          }
        }
...
int GSVPoseOptimization::
CreateMeshCorrespondences(void)
{
  // Initialize parameters
  const int max_iterations = 32;
  int saved_features = NFeatures();
  int saved_correspondences = NCorrespondences();
  RNScalar saved_max_correspondence_world_distance = max_correspondence_world_distance;
  RNScalar saved_max_correspondence_image_distance = max_correspondence_image_distance;
  RNScalar max_world_distance = icp_max_world_distance_start;
  RNScalar max_image_distance = icp_max_image_distance_start;
  RNScalar correspondences_per_cubic_meter = 1;

  // Iteratively create correspondences
  for (int i = 0; i < max_iterations; i++) {
    // Create correspondences
    TruncateFeatures(saved_features);
    TruncateCorrespondences(saved_correspondences);
    max_correspondence_world_distance = max_world_distance;
    max_correspondence_image_distance = max_image_distance;
    CreateMeshCorrespondences(correspondences_per_cubic_meter);
...
ParseOption(const char *keyword, const char *value) 
{
  ...
  else if (!strcmp(keyword, "camera_camera_rigidity_weight"))
...
UpdatePixelPositionsFromCorrespondenceSolutions(void) 
{
  ...
  for (int i = 0; i < NFeatures(); i++) {
    ...
    if (feature->feature_type == GSV_IMAGE_LINE_FEATURE_TYPE) {
      R2Point image_pointA = feature->image_position - feature->image_scale * feature->image_direction;
      R2Point image_pointB = feature->image_position + feature->image_scale * feature->image_direction;
      R3Ray rayA = feature->image->RayThroughUndistortedPosition(image_pointA);
      R3Ray rayB = feature->image->RayThroughUndistortedPosition(image_pointB);
      R3Point scan_pointA = rayA.Point(feature->image_t);
      R3Point scan_pointB = rayB.Point(feature->image_t);
      feature->scan_normal = rayA.Vector() % rayB.Vector(); 
      feature->scan_normal.Normalize();
      feature->scan_direction = scan_pointB - scan_pointA;
      feature->scan_direction.Normalize();
      feature->scan_scale = 0.5 * R3Distance(scan_pointA, scan_pointB);
    }
  }
...
ComputeRayIntersection(GSVPoseOptimization *optimization,
  ...
  R3Point& intersection_position0, R3Point& intersection_position1, 
  RNScalar& intersection_t0, RNScalar& intersection_t1)
{
  // Swap features if necessary so code can be written once per pair of feature types
  GSVFeature *swap = NULL;
  if (feature0->feature_type > feature1->feature_type) {
    swap = feature0;
    feature0 = feature1;
    feature1 = swap;
  }
  ...
  int status = 0;
  if (((feature0->feature_type == GSV_SCAN_POINT_FEATURE_TYPE) && (feature1->feature_type == GSV_IMAGE_POINT_FEATURE_TYPE)) ||
      ((feature0->feature_type == GSV_SCAN_POINT_FEATURE_TYPE) && (feature1->feature_type == GSV_IMAGE_LINE_FEATURE_TYPE))) {
    R3Point point0 = feature0->scan_position;
    ...
    if (RNIsPositive(t)) {
      intersection_position0 = point0;
      intersection_position1 = ray1.Point(t);
      intersection_t0 = 0;
      intersection_t1 = t;
      status = 1;
    }
  }
  else if (((feature0->feature_type == GSV_SCAN_LINE_FEATURE_TYPE) && (feature1->feature_type == GSV_IMAGE_POINT_FEATURE_TYPE)) ||
           ((feature0->feature_type == GSV_SCAN_LINE_FEATURE_TYPE) && (feature1->feature_type == GSV_IMAGE_LINE_FEATURE_TYPE))) {
    R3Ray ray0(feature0->scan_position, feature0->scan_direction);
    R3Ray ray1 = feature1->image->RayThroughUndistortedPosition(feature1->image_position);
    ...
    if (RNIsNotZero(denom)) {
      ...
      intersection_position0 = ray0.Point(t0);
      intersection_position1 = ray1.Point(t1); 
      intersection_t0 = 0;
      intersection_t1 = t1;
      status = 1;
    }
  ...
  // Swap result if swapped features
  if (swap) {
    R3Point p = intersection_position0; intersection_position0 = intersection_position1; intersection_position1 = p;
    RNScalar t = intersection_t0; intersection_t0 = intersection_t1; intersection_t1 = t;
  }
...
UpdatePixelPositionsFromRayIntersections(void) 
{
  ...
  for (int i = 0; i < NCorrespondences(); i++) {
    ...
    if ((feature0->feature_type == GSV_IMAGE_POINT_FEATURE_TYPE) ||
        ...
        (feature0->feature_type == GSV_IMAGE_LINE_FEATURE_TYPE) ||
        (feature1->feature_type == GSV_IMAGE_LINE_FEATURE_TYPE)) {
      RNScalar t0 = 0, t1 = 0;
