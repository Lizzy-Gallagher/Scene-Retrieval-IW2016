...
ParseRoom(P5DRoom *room, Json::Value *json_room, int item_index, int idx_index)
{
  ...
  if (GetJsonObjectMember(json_value, json_room, "className", Json::stringValue)) { 
      room->className = (char*) malloc(strlen(json_value->asString().c_str())+1);
      strcpy(room->className, json_value->asString().c_str()); 
  ...
  // Temporary solution: store all the rtypes as a delimited string. Then split
  // in the client code.
  std::string delimiter = ",";
  std::string rtypeStr;
  if (!GetJsonObjectMember(json_items, json_room, "rtypeStr", Json::arrayValue)) return 0;
  for (Json::ArrayIndex index = 0; index < json_items->size(); index++) {
    if (!GetJsonArrayEntry(json_item, json_items, index)) continue; // return 0;
    rtypeStr += json_item->asString() + delimiter;
  }
  room->rtypeStr = strdup(rtypeStr.c_str());
