...
static const char *input_prefix = NULL;
...
LoadProjectIds(std::string filename = "data/list-of-projects.txt") {
    ...
    if (project_ids_file.is_open())
    {
        ...
        {
            std::string s;
            s = input_prefix + id + "/project.json";
            project_ids.push_back(s);
        }
...
CreateSurfaceGrid(R3SceneNode *node, R3Box bbox, RNScalar scale)
{
    ...
    //fprintf(stdout, "xres: %d yres: %d zres: %d\n", xres, yres, zres);
...
ParseArgs(int argc, char **argv)
{
    ...
    while (argc > 0) {
        if ((*argv)[0] == '-') {
            ...
            if (!input_prefix) input_prefix = *argv;
            else if (start_idx == -1) start_idx = atoi(*argv);
            else if (end_idx == -1) end_idx = atoi(*argv);
    ...
    if (!input_prefix || !output_relationship_name) {
        fprintf(stderr, "Usage: scn2rel inputprefix startidx endidx outputrelationshipsfile [options]\n");
        return 0;
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = start_idx; i < end_idx; i++) {
        ...
        fprintf(stdout, "Reading %s", project_id.c_str());
